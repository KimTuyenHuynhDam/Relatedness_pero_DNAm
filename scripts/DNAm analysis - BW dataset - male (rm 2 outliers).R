...
nested_anova <- complete_data %>%
  ...
  mutate(
    ...
    reduced_model = map(data, ~ lm(Beta ~ Age, data = .)),
    full_model = map(data, ~ lm(Beta ~ Relatedness + Age, data = .)),
...
write.xlsx(nested_anova_final, "./DNAm_BW/Males only - nested_anova_final_hu_pman_2.1.100_anno (test).xlsx")


write.xlsx(nested_anova_final, "./DNAm_BW/Males only - nested_anova_final_hu_pman_2.1.100_anno (rm 2 outliers).xlsx")
