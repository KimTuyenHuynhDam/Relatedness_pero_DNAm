...
DNAm    <- read.xlsx(DNAM_PATH) %>% clean_column_names() %>%
  dplyr::select(Species, ExternalSampleName, Age, Sex, CorrRatioResAge)
...
for (species in TARGET_SPECIES) {
  ...
  for (i in seq_len(nrow(DNAm_sp))) {
    ...
    res[[i]] <- tibble(
      ...
      Age = r$Age,
      Sex = r$Sex,
      CorrRatioResAge = r$CorrRatioResAge,
      SampleID = sid,
  ...
  if (length(sample_ids_norm) < 2) {
    ...
    # --- MODIFIED: Renamed 'rel_all' to 'rel_all_df' ---
    rel_all_df <- IND %>%
    ...
    missing_samples <- setdiff(sample_ids_norm, rel_all_df$ID) # <--- MODIFIED
    ...
    all_parent_ids <- unique(stats::na.omit(c(rel_all_df$Dam, rel_all_df$Sire))) # <--- MODIFIED
    missing_parents <- setdiff(all_parent_ids, rel_all_df$ID) # <--- MODIFIED
    if (length(missing_parents) > 0) {
      is_sire <- missing_parents %in% rel_all_df$Sire # <--- MODIFIED
      is_dam  <- missing_parents %in% rel_all_df$Dam  # <--- MODIFIED
      sex_infer <- ifelse(is_sire & !is_dam, 1L, ifelse(is_dam & !is_sire, 2L, 0L))
      rel_all_df <- bind_rows( # <--- MODIFIED
        rel_all_df, # <--- MODIFIED
        tibble(
    ...
    id_chr  <- as.character(rel_all_df$ID) # <--- MODIFIED
    dad_chr <- ifelse(is.na(rel_all_df$Sire) | rel_all_df$Sire == 0, NA_character_, as.character(rel_all_df$Sire)) # <--- MODIFIED
    mom_chr <- ifelse(is.na(rel_all_df$Dam)  | rel_all_df$Dam  == 0, NA_character_, as.character(rel_all_df$Dam)) # <--- MODIFIED
    
    keep <- !is.na(id_chr) & nzchar(id_chr)
    id_chr  <- id_chr[keep]
    dad_chr <- dad_chr[keep]
    mom_chr <- mom_chr[keep]
    sex_vec <- rel_all_df$Sex_k2[keep] # <--- MODIFIED
    ...
    if (length(target_ids_present) < 2) {
      ...
      # ----------------- HEATMAP BY CORRRATIORESAGE (w/ ANNOTATION) ------------------
      message(glue("[{species}] Generating heatmap by CorrRatioResAge..."))
      
      # 1. Get ordering data
      ordering_data_crr <- out_df %>%
        filter(SampleID %in% target_ids_present) %>%
        distinct(SampleID, .keep_all = TRUE) %>% 
        arrange(CorrRatioResAge)
      ...
      g_annot_crr <- ggplot(plot_data_annot_crr, aes(x = ExternalSampleName, y = 1, fill = CorrRatioResAge)) +
        geom_tile(color = "grey50", linewidth = 0.1) +
        # Use a different, nice color scale for the annotation
        scale_fill_distiller(palette = "RdYlBu", name = "CorrRatio\nResAge") +
        ...
        labs(
          x = "Sample", 
          y = NULL,
          subtitle = glue("Ordered by CorrRatioResAge (n={length(ordered_names_crr)}, depth={GENERATION_DEPTH} gen)")
        ) +
        theme(
      ...
      heatmap_path_crr <- file.path(OUT_DIR, glue("{species} - DNAm relatedness heatmap by CorrRatioResAge (gen {GENERATION_DEPTH}).png"))
      ...
      # Set the main matrix_ordered variable for saving to Excel
      matrix_ordered <- matrix_ordered_crr
    }
  }
  ...
  if (!is.null(matrix_ordered)) {
    addWorksheet(wb, "AllPairsMatrix_by_CRR")
    # This saves the matrix ordered by CorrRatioResAge
    matrix_df <- as.data.frame(matrix_ordered) %>%
      tibble::rownames_to_column("Sample")
    writeData(wb, "AllPairsMatrix_by_CRR", matrix_df, rowNames = FALSE)
  }
